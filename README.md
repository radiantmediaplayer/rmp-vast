...
It aims to closely implement the [IAB VAST 3 specification](https://www.iab.com/guidelines/digital-video-ad-serving-template-vast-3-0/) for web-based environments 
(e.g. browser, WebView ...) where both HTML5 video and JavaScript are available.
rmp-vast is used in [Radiant Media Player](https://www.radiantmediaplayer.com/) for ad playback capabilities.
rmp-vast is built with ES2015 JavaScript and ported to ES5 JavaScript with Babel.
rmp-vast is an open-source project released under MIT license.

## Supported VAST 3 features
- Inline and Wrapper Ads
- Linear Ads (MP4/WebM)
- Skippable Linear Ads (MP4/WebM)
...
enough coverage to support current industry requirements and best pratices.

## Additional VAST-realted supported features
...
## QuickStart guide
First download latest rmp-vast from the release tab.
...
- Include rmp-vast.min.js
- In order to use rmp-vast one must adhere to a specific HTML layout pattern. This pattern is as follows. 
```
...
You should look at app/js/app.js for a complete implementation example.

### Creating a rmp-vast instance
Once rmp-vast.min.js is loaded on your page you can create a new rmp-vast instance as follows:
...
`params.maxNumRedirects: Number` the number of VAST wrappers the player should follow before triggerring an error. Default: 4.
...
`params.textForClickUIOnMobile: String` on mobile devices the clickthrough URL for a linear ad is provided in a box located at the top right corner of the player. This setting set the text for this box. Default: 'Learn more'.

### Starting the rmp-vast player
It is important that the rmp-vast instance is properly initialized to avoid playback issues. Playing video ads in HTML5 video is a non-trivial process that requires the overlaying of multiple video tags or changes in source depending on the targeted environments. On mobile devices a user interaction is required to properly initialized a video tag and various restrictions are set by the OS to limit the manipulation of a video tag with JavaScript.
...
### API events
rmp-vast will fire VAST-related events on the player container as they occur. Events are registered and unregistered with the addEventListener and removeEventListener methods set on the player container. Example:
```
var id = 'rmpPlayer';
var container = document.getElementById(id);
...
...
- getAdLinear(): return a boolean representing the type of the creative either linear (true) or non linear (false)
...
- getClickThroughUrl(): return a string representing the clickthrough (e.g. destination) URL for the selected creative
...
- getIsUsingContentPlayerForAds(): on iOS (which we loved) the VAST player is the content player. This is to avoid fullscreen management issues and a consistent user experience. This method will return true for iOS, false otherwise
